...
public class Ground : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        screenRight = camera.transform.position.x * 2;
    }

    private void Update()
    {
        groundHeight = this.transform.position.y + (collider.size.y / 2);
    }
    ...
    private void FixedUpdate()
    {
        ...
        groundRight = this.transform.position.x + (collider.size.x / 2);
    ...
    private void generateGround()
    {
        ...
        pos.y = actualY - goCollider.size.y / 3;
        if (pos.y > 22.4f)
            pos.y = 22.4f;
        ...
        {          
            ...
            fall.fallSpeed = UnityEngine.Random.Range(1.0f, 3.0f);
        }

        int obstacleNum = UnityEngine.Random.Range(0, 2);

        for (int i = 0; i < obstacleNum; i++)
        {
            GameObject box = Instantiate(boxTemplate.gameObject);
            float y = goGround.groundHeight - 1.4f;
